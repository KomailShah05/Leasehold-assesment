...
const request = async <T>(path: string, body?: TriageRequest): Promise<ApiResult<T>> => {
  ...
  if (!response.ok) {
    ...
    const message = response.status === 400 && isErrorPayload(payload) ? payload.message : undefined
    return {
      ok: false,
...
export const submitTriage = (body: TriageRequest): Promise<ApiResult<TriageResponse>> =>
